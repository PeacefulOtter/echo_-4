...
window.onload = function( e ) {
  ...
  fetch( API_URL + 'users', {
    ...
    // Add each date into an array
    // but check first if the date before is one day before the next one
    // so we don't have a hell lot of dates
    /*let arrayDates  = [];
    let preDay = 0;
    player[0].dates.forEach( function( date ) {
      let clearDate = date.split(' ')[0].replace(/-/g, '/');
      let newDay = parseInt( clearDate.split('/')[2], 10 );
      if ( newDay <= preDay ) return;
      preDay = newDay;
      arrayDates.push( clearDate );
    } )
    ...
    new Chartist.Line(
      ...
      // data
      {
        labels: player[0]['dates'],//arrayDates,
        series: [
          playerS[ 'life' ]['kills']
        ]
      },
